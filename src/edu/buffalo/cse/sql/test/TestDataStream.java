...
public class TestDataStream implements Iterator<Datum[]> {
  ...
  public boolean validate(Iterator<Datum[]> inStream, Datum[] from, Datum[] to)
  {
    ...
    if(inStream.hasNext()){
    	found=inStream.next();
    	if (!hasNext() && Datum.compareRows(found, expected)==0){} //I think here there is a bug when expected is the last record in the data stream, because hasnext() is false, it is not compared with the found value
    	else {
      System.err.println("Provided stream is too long");
      return false;}
    }
  ...
  protected void stepAllKeys(){
    ...
    for(int i = guaranteeKeyStep ? 0 : 1; i <= steps; i++){// dont think guaranteeKeyStep is true will guarantee this happen, false will happen
      stepOneKey(reset);
      reset = false;
    }
  }
