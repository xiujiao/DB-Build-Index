...
ManagedFile mf1;
    ...
    throws SqlException, IOException
  {
	  ...
	  int start=0;
	  int end=0;
	  ArrayList <Datum []> records=new ArrayList <Datum []> ();//record all the records
	  ...
	  int le1=d1.length;//length of one record
	  Datum [] k1=key.createKey(d1);
	  int le2=k1.length;//length of the key
	  Datum d22=d1[le2];
	  d1[le2]=new Datum.Int(0);// replace the original value in record (useless) and mark it as the lowest level in the tree
...
//		  Datum [] d2=db.readreserve();
	  ...
	  /////////////////////////////////////////////the upper part is to calculate pa//////////////
	  ...
	  for (int i1=0;i1<pa;i1++){
		  ...
		  for (int j1=0;j1<re1;j1++){
			  if (i3<re){
			  d1=records.get(i3);
			  if (flag==0){						// the reserved space will store the key of the first record now
				  d22=d1[le2];
...
//		  Datum [] d2=db3.readreserve();
		  ...
		  pa1=Math.ceil((pa1-1)/re1);// (pa1-1) is the number of keys information for the upper level of the tree 
...
//				  DatumBuffer db2=new DatumBuffer(mf.getBuffer(start+i3),sch);
				 // mf.pin(start+i3);
				  DatumBuffer db2=new DatumBuffer(mf.safeGetBuffer(start+i3),sch);
//				  mf.pin(start+i3);///////////////////////////////////////this is very important, because when the frames are only set as 10, it will crash when 9 pages are open without being set free
				  int ire3=db1.remaining();
				  d1=db2.readreserve();
				  
				  db2.initialize();
//				  mf.unpin(start+i3);///////////////////////////////////////this is very important, because when the frames are only set as 10, it will crash when 9 pages are open without being set free
				//  mf.unpin(start+i3);
//				  mf.flush();
 				  ire3=db1.remaining();
				  
//				  DatumBuffer db23=new DatumBuffer(mf.safeGetBuffer(start+i3),sch);
//				  Datum [] d12=db2.readreserve();
				  int ire4=db1.remaining();
				  if (flag==0){
					  d22=d1[le2];
					  d1[le2]=new Datum.Int(level);
					  db1.writereserve(d1);
...
//	  Datum [] d5=db4.readreserve();
    ...
    throws SqlException, IOException
  {
	  ...
	  for (int i=0;i<pa;i++){
//		  mf1.pin(i);
		  DatumBuffer rootBuffer=new DatumBuffer(mf1.getBuffer(i),key1.rowSchema());
		  Datum [] rootDatum=rootBuffer.readreserve();
		  Datum [] k2=key1.createKey(rootDatum);
...
//		  mf1.pin(i);
		  ...
		  {
			  DatumBuffer rootBuffer1=new DatumBuffer(mf1.getBuffer(i-1),key1.rowSchema());
		  ...
		  {
			  DatumBuffer rootBuffer1=new DatumBuffer(mf1.getBuffer(i),key1.rowSchema());
			  for (int j=0; j<rootBuffer1.length();j++)
			  {
				  Datum [] re=rootBuffer1.read(j);
				  Datum [] k3=key1.createKey(re);
...
//	  IndexIterator ini=(IndexIterator)in;
  ...
  public Datum[] getTo(Datum[] key)
		    throws SqlException, IOException
		  {
			  ...
			  DatumBuffer rootBuffer=new DatumBuffer(mf1.getBuffer(pa-1),key1.rowSchema());
			  Datum [] rootDatum=rootBuffer.readreserve();
			  Datum [] k1=key1.createKey(rootDatum);
			  root=rootDatum[k1.length].toInt();
			  Datum [] min=new Datum [k1.length];
			  for (int j=0; j<k1.length;j++) // initialize the min to record the most closest key to the wanted key
			  {
			  ...
			  for (int i=pa-1;i>=0;i--){
				  ...
				  if (node==root){
					  ...
					  if (cmp==1){
						  ...
						  if (cmp1==1 || cmp1==0){
							  min=k2;              //find the most closest key in the current level of the tree to the wanted key
							  flag=i;
						  }
					  }
					  else if (cmp==0)
			  ...
			  if (flag!=-1){
				  ...
				  for (int i1=0;i1<rootBuffer.length();i1++){
					  ...
					  if (cmp==1){
						int cmp1=key1.compare(k2,min);
							if (cmp1==1 || cmp1==0){
								  min=k2;              //find the most closest key in the next level of the tree to the wanted key
							  }
						 	}
					  else if (cmp==0)
					  {min=k2;
			  ...
			  for (root=root;root>=0;root--){
				  ...
				  for (int i=pa-1;i>=0;i--){
					  ...
					  if (node==root){
						  ...
						  if (cmp==0) //here is a little different,because this must exist
						  {
							  flag=i;	//find the most closest key in the current level of the tree to the wanted key
							  break;
						  }
					  }
					  if (node<root)
  ...
  public Datum[] getFrom(Datum[] key)
		    throws SqlException, IOException
		  {
				  ...
				  for (int i1=0;i1<rootBuffer.length();i1++){
					  ...
					  if (cmp==-1){
						int cmp1=key1.compare(k2,max);
							if (cmp1==-1 || cmp1==0){
								  max=k2;             //find the smallest key that are larger than the current key
							  }
						 	}
					  else if (cmp==0)
					  {max=k2;
...
//			  throw new SqlException("Unimplemented");
    ...
    throws SqlException, IOException
  {
	 ...
	 // int a1=a.hashCode();
	  ...
	  DatumBuffer rootBuffer1=new DatumBuffer(mf1.getBuffer(0),key1.rowSchema());
