...
public class DatumBuffer {
  ...
  protected Datum read(int position, Schema.Type t)
  ...
  //write the key and sth into the reserved space
  public int writereserve (Datum[] row){
	 int start=0;
	 int i=DatumSerialization.write(buffer, start, row);
	 //start=start+i;
	 //i=DatumSerialization.write(buffer,start,d);
	 return i;
  }
  // read the key and sth from the reserved space
  public Datum[] readreserve ()
  {
	  return DatumSerialization.read(buffer, 0, schema);
  }
...
//    System.out.println("Used "+(i+4)+" bytes; now remaining "+remaining()+" bytes");
  ...
  protected int binarySearch(int start, int length, Comparable<Datum[]> target)
  {
  ...
  public int find(Comparable<Datum[]> target)
    { return binarySearch(0, length(), target); }

  public int find(final Datum[] target)
  {
